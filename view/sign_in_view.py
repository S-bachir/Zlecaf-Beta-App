...
class SignInView(AbstractView):
    ...
    def make_choice(self):
        ...
        try:
            ...
            try:
                super_pseudo = SuperUtilisateurService.get_super_user_by_name(reponse1['pseudonyme'])[0]
                if reponse1['pseudonyme'] == super_pseudo:
                    AbstractView.session.user_name = reponse1['pseudonyme']
                else:
                    pass
                ...
                mdp_sign = reponse2['mot de passe'].encode()
                mdp_sign_hash = sha1(mdp_sign).hexdigest()
                mdp_hash_super = SuperUtilisateurService.get_super_user_by_name(reponse1['pseudonyme'])[1]
                
                if mdp_sign_hash == mdp_hash_super:
                    AbstractView.session.user_mdp = reponse2['mot de passe']
